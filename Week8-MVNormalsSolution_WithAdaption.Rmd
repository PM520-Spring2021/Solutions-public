...
```{r}
samp.1a <- MCMC(lupost, n=total.iterations, init=c(0, 1), scale=c(1, 0.1),
               adapt=FALSE)
samp.1b <- MCMC(lupost, n=total.iterations, init=c(2, 2), scale=c(1, 0.1),
               adapt=FALSE)
```
...
These diagnostics are actually fine! What was our effective sample size?...
```{r}
effectiveSize(bothsamps1)
```

So, out of 20K iterations we have around 100 effectively independenty samples. 
...
You can see that there is a bit of a burn in period, which we should remove before using the samples.
